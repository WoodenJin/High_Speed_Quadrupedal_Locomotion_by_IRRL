...
class PPO2(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input_info", reuse=False):
                    ...
                    tf.summary.scalar(
                        'old_neglog_action_probabilty', tf.reduce_mean(self.old_neglog_pac_ph))
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            runner = Runner(env=self.env, model=self,
                            n_steps=self.n_steps, gamma=self.gamma, lam=self.lam)
                ...
                try:
                    ...
                    if update % eval_every_n == 1:
                        ...
                        obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward = \
                            runner.run(test_mode=True, record_video=record_video,
                                       video_name=log_dir + "/" + str(update - 1) + ".mp4")
                        print("Average rewards in this test episode ",
                              ep_infos[0]['r'])
                        model_name = log_dir + \
                            "_Iteration_{}".format(update - 1)
                    ...
                    # print("masks:", masks)
                    # print("masks shape:", masks.shape)  # mask 的长度和value一样
                    # print("actions:", actions)
                    # print("values:", values)
                    # print("values shape:",values.shape)
                    # print("neglo:", neglogpacs)
                    # print("states:", states)
                    ...
                    if states is None:  # nonrecurrent version
                        ...
                        self.num_timesteps += (self.n_batch *
                                               self.noptepochs) // batch_size * update_fac
                        ...
                        self.num_timesteps += (self.n_envs *
                                               self.noptepochs) // envs_per_batch * update_fac
    ...
    def save(self, save_path):
        ...
        self._save_to_file(save_path, data=data,
                           params=params, cloudpickle=True)
    ...
    def run(self, test_mode=False, record_video=False, video_name=""):
        ...
        for _ in range(self.n_steps):
            ...
            self.obs[:], rewards, self.dones, infos = self.env.step(
                clipped_actions, visualize=test_mode)
            for info in infos:
        ...
        mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward = \
            map(swap_and_flatten, (mb_obs, mb_returns, mb_dones,
                mb_actions, mb_values, mb_neglogpacs, true_reward))
